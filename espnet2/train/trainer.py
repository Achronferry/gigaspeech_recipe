...
from espnet2.main_funcs.average_nbest_models import average_nbest_models,save_model,update_best,remove_nbest
...
class Trainer:
    ...
    def run(
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            if "SAVE" not in dry_run and (not distributed_option.distributed or distributed_option.dist_rank == 0):
                ...
                if (
                    ...
                    logging.info(f"Average nbest model:{keep_nbest_models} by {trainer_options.best_model_criterion}")
    ...
    def train_one_epoch(
        ...
        for iiter, (_, batch) in enumerate(
            ...
            try:
                ...
                if distributed:
                    state = "all_reduce"
                    with reporter.measure_time('all_reduce_time'):
                        torch.distributed.all_reduce(iterator_stop, ReduceOp.SUM)
                        if iterator_stop > 0:
                            break

                with reporter.measure_time("forward_time"), autocast(scaler is not None):
                    state = "forward"
                ...
                logging.error("Speech " + str(batch["speech"].shape) + " Text " + str(batch["text"].shape))
                ...
                if 'out of memory' in str(e):
                    ...
                    raise e
            # finally:
            #     logging.info("Speech " + str(batch["speech"].shape) + " Text " + str(batch["text"].shape))
