...
class DNN_Beamformer(torch.nn.Module):
    ...
    def __init__(
        ...
        if btype != "mvdr":
            raise ValueError(
                "Not supporting beamformer_type={}".format(btype)
            )
        self.beamformer_type = btype

    def forward(
        self, data: ComplexTensor, ilens: torch.LongTensor, masks: Tuple[torch.Tensor, ...]
        ...
        Args:
            data (ComplexTensor): (B, C, T, F)
            mask Tuple(FloatTensor): n, (B, C, T, F)
        ...
        # data (B, C, T, F) -> (B, F, C, T)
        data = data.permute(0, 3, 1, 2)
        # mask: (B, C, T, F) -> (B, F, C, T)
        assert self.nmask == len(masks)
        masks = [i.permute(0, 3, 1, 2) for i in masks]
