...
my $max_jobs_run;
my $gpu = 0;
my $force_quit = 0; # if force quit, quit all jobs if one failed; else quit as normal.
...
sub print_usage() {
  print STDERR
   ...
   "  --force_quit 0/1 (quit if failed one job, default=1)\n" .
...
sub caught_signal {
  if ( defined $sge_job_id ) { # Signal trapped after submitting jobs
    ...
    if ($force_quit == 1) {
      ...
      die "\e[01;31mERROR: Caught a signal: $signal , deleting SGE task: $sge_job_id and exiting\e[0m\n";
    } else {
      die "\e[01;31mERROR: Caught a signal: $signal , error job status\n\e[0m";
    }
  }
...
default gpu=0
option gpu=0 -p cpu
option gpu=* -p gpu --gres=gpu:$0 # this has to be figured out
EOF
...
for my $option (keys %cli_options) {
  ...
  if ($option eq "force_quit") {
    $force_quit = $cli_options{$option};
  } elsif ($option eq "max_jobs_run") {
    if ($array_job != 1) {
    ...
    while (! -f $f) {
      ...
      if (($check_sge_job_ctr++ % 10) == 0) { # Don't run qstat too often, avoid stress on SGE.
        ...
        if ( $output =~ "FAIL" || $output =~ "CANCEL" ){
            print STDERR "Some jobs failed, suppose to be shut down \n";
            print STDERR $output;
            caught_signal();
            last;
        }
