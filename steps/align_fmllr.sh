...
if [ $stage -le 4 ]; then
  echo "$0: ALIGNMENT ==========>>>> $data/ali.scp"
  $cmd JOB=1:$nj $dir/log/align_pass2.JOB.log \
    gmm-align-compiled $scale_opts --beam=$beam --retry-beam=$retry_beam --careful=$careful "$mdl_cmd" \
    "ark:gunzip -c $graphdir/fsts.JOB.gz|" "$feats" "ark,scp:$sdata/JOB/ali.ark,$sdata/JOB/ali.scp" || exit 1;
  cat $sdata/JOB/ali.scp > $data/ali.scp
fi
